...
- **2026-08-21**: Added and then substantially expanded [[Motivation - Why Verifying the Verifier Matters]] — the justification note for the direction under discussion (is verifier soundness a real, at-scale, still-open problem?). Beyond the CVE/impact evidence, it now carries four things the literature review did not: (a) **the 2026 landscape shift** — at LSFMM+BPF 2026 the BPF maintainers began re-drawing the verifier's trust boundary in public (Starovoitov splitting it into "catch programmer mistakes" vs "security boundary" and proposing loop **widening that trades static precision for inserted runtime checks**; Zingerman's **prototype scalar-evolution loop analysis** — an optimizing compiler inside the TCB, with an acknowledged unfixed inner/outer-loop ordering defect; Dwivedi's Verus domain-invariants proposal, mixed reception); (b) an inventory of **buildable artifacts** (Agni, the 67-PoC bug corpus, PREVAIL, bpf_conformance/DiffSpec, the 235-rejection bpfix dataset) with their real setup costs; (c) the **funding/venue surface** — eBPF Foundation grants name this topic explicitly but are **faculty-PI only**, and eBPF '26 has **moved from SIGCOMM to SOSP** (Prague, 2026-09-29) with a 2-page poster track that is the realistic capstone on-ramp for **eBPF '27**; (d) **honest counter-arguments**, including that the ecosystem's own direction (runtime checks + Rust-by-construction) partly undercuts a naive "prove the verifier sound" framing. **Consequence for the direction in the 2026-08-07 entry**: the oracle-comparison artifact is now *easier* (a shared corpus exists that did not before) but should be framed against the new runtime-check story rather than the 2023-era static one; the newly-prototyped loop machinery is the least-claimed alternative. Also resolved the outstanding ⚠️ on the "56 eBPF CVEs, majority in the verifier" figure — primary source is Fig. 1 of arXiv:2308.01983. New sources are listed at the foot of that note for triage into [[1 - Formal Verification of the Verifier and JIT]]; **they have not yet been numbered into Section 1.**
